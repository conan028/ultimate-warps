...
object PermUtil {
    ...
    fun commandRequiresPermission(source: ServerCommandSource, permission: String): Boolean {
        ...
        return player != null
                && (source.hasPermissionLevel(2) || isLuckPermsPresent()
                && getLuckPermsApi()?.userManager?.getUser(player.uuid)?.cachedData?.permissionData?.checkPermission(permission)?.asBoolean() == true) || source.entity == null
    }
}